...
func (x *xrgrpc) checkConfig(json string) error {
	...
	for {
		...
		if len(r.GetErrors()) != 0 {
			return fmt.Errorf("get config error triggered by remote host for ReqId: %v; %s", id, r.GetErrors())
		}
	...
	if len(r.GetErrors()) != 0 {
		return fmt.Errorf("delete error triggered by remote host for ReqId: %v; %s", id, r.GetErrors())
	}
	return nil
}
...
func main() {
	// falg to enable self-describing subscription messages
	...
	iosxr := xrgrpc{
		IOSXR: IOSXR{
			Hostname: "sandbox-iosxr-1.ciscio.com",
			Authentication: Authentication{
				Username: "admin",
				Password: "Ciscio12345",
			},
		},
	}
	...
	// Hence, delete any existing BGP config first
	...
	subMode := "gpb"
	if *kvMode {
		fmt.Printf("self-describing kv subscription mode")
		subMode = "gpbkv"
	}
	...
	// If no signals are provided, all incoming signals will be replayed to c.
	// Otherwise, just the provided signals will. E.g.: signal.Notify(c, os.Interupt, syscall.SIGTERM)
	...
	for msg := range ch {
		...
		for _, row := range message.GetDataGpb().GetRow() {
			...
			if err != nil {
				fmt.Printf("could decode context: %v\n", err)
				return
			}
