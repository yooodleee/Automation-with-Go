...
func (r SRL) GetRoutes(wg *sync.WaitGroup) {
	...
	if err != nil {
		log.Printf("failed to create for %s: %s", r.Hostname, err.Error())
		return
	}
	...
	err = driver.Open()
	if err != nil {
		log.Printf("failed to open driver %s: %s", r.Hostname, err.Error())
		return
	}
	defer driver.Close()
...
func (r CEOS) GetRoutes(wg *sync.WaitGroup) {
	...
	template := "https://raw.githubusercontent.com/networktocode/ntc-templates/templates/arista_eos_show_ip_route.textfsm"
